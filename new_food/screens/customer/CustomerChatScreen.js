...
function CustomerChatScreen({ navigation, route }) {
    const { idRoom, restaurantName } = route.params;
    const myId = 1;
    ...
    useEffect(() => {
        const demoList = [
            ...
            },
            {
                id: 2,
                userId: 0,
                content: "Đây là chat customer, Nhớ rằng các đối tượng được gửi qua navigation params cần phải là các giá trị có thể được serialize, ví dụ: các đối tượng JavaScript hoặc các kiểu dữ liệu cơ bản như số, chuỗi, mảng, hoặc đối tượng. Đối tượng không thể serialize như hàm không thể được gửi qua navigation params."
            },
        ];
    ...
    return (
        <View>
            <View className="p-4 bg-white flex-row">
                <TouchableOpacity
                ...
                <Text className="self-center text-xl font-bold ml-2">{restaurantName}</Text>
            </View>
